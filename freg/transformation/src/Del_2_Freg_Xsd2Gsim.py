...
tree = ET.parse('.//xsdFiles//Del_1_PersondokumentMedHistorikk.xsd')
root = tree.getroot()
...
# så er det nesten(?) 100% sikkert at dette er en FREG-kodeliste.
...
# Henter alle elementer i FREG-kodelisten
...
# Eksempelvis arv fra de abstrakte FREG-typene "Registermetadata" og "RegistermetadataMedGyldighet"
...
# TODO TODO TODO TODO:
#  Det må avklares hva som egentlig er en "primærnøkkel" (Identifiers) innenfor en FREG-meldingstype!
...
def addFregGsimIdentifierVariables():
    global gsimInstanceVariables
    instVar = {}
    instVar["id"] = str(uuid.uuid4())  # InstanceVariable.id
...
def iterateXsd(fregMeldingstype):
    global numOfInstanceVariables
    global sisteFregMeldingstype
    global gruppePrefix
    global sisteObject
    if gruppePrefix == "":
        #print("#### LogicalRecord: " + fregMeldingstype + " ####")
        print(" (Type: " + fregMeldingstype + ") ####")
    baseExtensionName = getBaseExtensionName(fregMeldingstype)
    if baseExtensionName is not None:
        if fregMeldingstype != sisteFregMeldingstype: # "if" for å unngå evig løkke for noen "selv-refererende objekter", f.eks. "Folkeregisterpersonnavn.originaltNavn"
            sisteFregMeldingstype = fregMeldingstype
    ...
    elements_1 = findElementsInObject(fregMeldingstype)
    if elements_1 is None:
        print("ERROR - UKJENT TYPE: " + fregMeldingstype)
    else:
...
def gsimCreateUnitDataSet(gsimSource):
    ...
    js["description"] = [{"languageCode": "nb", "languageText": "FREG datasett " + splitStringOnUpperCase(gsimSource.get("dataSetName"))}]
...
def gsimCreateUnitDataStructure(gsimSource):
    ...
    js["description"] = [{"languageCode": "nb", "languageText": "FREG datastruktur " + splitStringOnUpperCase(gsimSource.get("unitDataStructureName"))}]
    js["administrativeStatus"] = "DRAFT"
    js["versionValidFrom"] = getZuluDateTime()
    js["validFrom"] = getZuluDateTime()
    js["createdDate"] = getZuluDateTime()
...
def gsimCreateLogicalRecord(gsimSource):
    ...
    js["description"] = [{"languageCode": "nb", "languageText": "FREG record (LogicalRecord) " + splitStringOnUpperCase(gsimSource.get("logicalRecordName"))}]
    js["administrativeStatus"] = "DRAFT"
    js["versionValidFrom"] = getZuluDateTime()
    js["validFrom"] = getZuluDateTime()
    js["createdDate"] = getZuluDateTime()
...
def gsimCreateInstanceVariables(gsimSource):
    ...
    for instVar in gsimSource.get("instanceVariables"):
        ...
        js["description"] = [{"languageCode": "nb", "languageText": "FREG " + gsimSource.get("logicalRecordName") + " " + getGruppePrefix(instVar).replace("_", " ") + splitStringOnUpperCase(instVar.get("name"))}]
        js["administrativeStatus"] = "DRAFT"
        js["versionValidFrom"] = getZuluDateTime()
        js["validFrom"] = getZuluDateTime()
        js["createdDate"] = getZuluDateTime()
...
def mappingRawDataToInputData(gsimSource):
    ...
    jsonPath = Path(targetPath + '_mapping')
...
def generateGsimJsonForFregFolkeregisterperson(fregMeldingstype=None):
    global gsimInstanceVariables
    global numOfInstanceVariables
    global sisteFregMeldingstype
    global gruppePrefix
    ...
    i = 0
    fregMelding = findXsdObjectByName(fregMeldingstype)
    i += 1
    print("\n#### LogicalRecord: " + fregMeldingstype, end='')
    iterateXsd(fregMelding.attrib.get("type"))
    gsimSource = {}
    gsimSource["dataSetName"] = fregMelding.attrib.get("name")
    ...
    gsimSource["unitDataStructureName"] = fregMelding.attrib.get("name")
    gsimSource["unitDataStructureId"] = str(uuid.uuid4())
    gsimSource["logicalRecordName"] = fregMelding.attrib.get("name")
    gsimSource["logicalRecordId"] = str(uuid.uuid4())
    gsimSource["instanceVariables"] = gsimInstanceVariables
    addFregGsimIdentifierVariables()
...
generateGsimJsonForFregFolkeregisterperson("falskIdentitet")
generateGsimJsonForFregFolkeregisterperson("identifikasjonsnummer")
generateGsimJsonForFregFolkeregisterperson("identitetsgrunnlag")
generateGsimJsonForFregFolkeregisterperson("kontaktinformasjonForDoedsbo")
generateGsimJsonForFregFolkeregisterperson("status")
generateGsimJsonForFregFolkeregisterperson("legitimasjonsdokument")
generateGsimJsonForFregFolkeregisterperson("utlendingsmyndighetenesIdentifikasjonsnummer")
generateGsimJsonForFregFolkeregisterperson("utenlandskPersonidentifikasjon")
generateGsimJsonForFregFolkeregisterperson("doedsfall")
generateGsimJsonForFregFolkeregisterperson("familierelasjon")
generateGsimJsonForFregFolkeregisterperson("sivilstand")
generateGsimJsonForFregFolkeregisterperson("bostedsadresse")
generateGsimJsonForFregFolkeregisterperson("deltBosted")
generateGsimJsonForFregFolkeregisterperson("oppholdsadresse")
generateGsimJsonForFregFolkeregisterperson("postadresse")
generateGsimJsonForFregFolkeregisterperson("postadresseIUtlandet")
generateGsimJsonForFregFolkeregisterperson("adressebeskyttelse")
generateGsimJsonForFregFolkeregisterperson("preferertKontaktadresse")
generateGsimJsonForFregFolkeregisterperson("innflytting")
generateGsimJsonForFregFolkeregisterperson("utflytting")
generateGsimJsonForFregFolkeregisterperson("forholdTilSametingetsValgmanntall")
generateGsimJsonForFregFolkeregisterperson("kjoenn")
generateGsimJsonForFregFolkeregisterperson("statsborgerskap")
generateGsimJsonForFregFolkeregisterperson("foreldreansvar")
generateGsimJsonForFregFolkeregisterperson("foedsel")
generateGsimJsonForFregFolkeregisterperson("navn")
generateGsimJsonForFregFolkeregisterperson("foedselINorge")
generateGsimJsonForFregFolkeregisterperson("vergemaalEllerFremtidsfullmakt")
generateGsimJsonForFregFolkeregisterperson("opphold")
generateGsimJsonForFregFolkeregisterperson("bibehold")
generateGsimJsonForFregFolkeregisterperson("brukAvSamiskSpraak")
